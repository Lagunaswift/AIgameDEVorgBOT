...
//
//   guidelines missing + no photo -> guidelines ask + photo ask + gentle reminder
//   guidelines missing + photo    -> guidelines ask + gentle reminder
//   guidelines ok + no photo      -> photo ask only (no gentle reminder: it's friendly)
...
export async function sendCombinedNudge(thread, ownerId, { cfg = config, services = { isGuidelinesExempt, threadFollowsGuidelines, threadHasScreenshot, guidelinesDedupRef, screenshotDedupRef } } = {}) {
  ...
  const wantGuidelines = cfg.guidelinesNudgeEnabled && !(await services.isGuidelinesExempt(thread));
  const wantPhoto = cfg.screenshotNudgeEnabled;

  let needsGuidelines = false;
  if (wantGuidelines) needsGuidelines = !(await services.threadFollowsGuidelines(thread));

  let needsPhoto = false;
  if (wantPhoto) needsPhoto = !(await services.threadHasScreenshot(thread));

  if (!needsGuidelines && !needsPhoto) return { sent: false, guidelines: false, photo: false };

  let current;
  try { current = await thread.fetch(); } catch { return { sent: false, guidelines: false, photo: false }; }
  if (!current || current.archived || current.ownerId !== ownerId) return { sent: false, guidelines: false, photo: false };
  if (needsGuidelines && await services.isGuidelinesExempt(current)) needsGuidelines = false;
  thread = current;
  ...
  const claimed = [];
  if (needsGuidelines) {
    const ref = services.guidelinesDedupRef(thread.id);
    if (await claim(ref, thread, ownerId)) claimed.push(ref);
    else needsGuidelines = false;
  }
  if (needsPhoto) {
    const ref = services.screenshotDedupRef(thread.id);
    if (await claim(ref, thread, ownerId)) claimed.push(ref);
